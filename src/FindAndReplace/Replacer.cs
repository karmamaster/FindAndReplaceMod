...
namespace FindAndReplace
{
	...
	public class Replacer
	{
        ...
        public bool WholeMatch { get; set; }
        public bool ObFuscate { get; set; }
        public string ReplaceText { get; set; }
        public string hashPool1 { get; set; }
        public string hashPool2 { get; set; }
        public string hashPool3 { get; set; }

        public string hashOrigPool { get; set; }
        ...
        public  string obFuscate(string obDataString)
        {
            string result = "";
            Random rndNum = new Random();
            int rndFlag = rndNum.Next(1, 3);
            string[] origPool = hashOrigPool.Select(x => x.ToString()).ToArray();
            IEnumerable<KeyValuePair<string, string>> resultArray = null;
            if (rndFlag == 1)
            {
                string[] hashPoolArray = hashPool1.Select(x => x.ToString()).ToArray();
                //resultArray = origPool.Zip(hashPoolArray, (origPool1, hashPoolArray1) => new KeyValuePair<string, string>(origPool1, hashPoolArray1));
                resultArray = origPool.Zip(hashPoolArray, (origPool1, hashPoolArray1) => new KeyValuePair<string, string>(origPool1, hashPoolArray1));
            }
            else if (rndFlag == 2)
            {
                string[] hashPoolArray = hashPool2.Select(x => x.ToString()).ToArray();
                resultArray = origPool.Zip(hashPoolArray, (origPool1, hashPoolArray1) => new KeyValuePair<string, string>(origPool1, hashPoolArray1));
            }
            else if (rndFlag == 3)
            {
                string[] hashPoolArray = hashPool3.Select(x => x.ToString()).ToArray();
                resultArray = origPool.Zip(hashPoolArray, (origPool1, hashPoolArray1) => new KeyValuePair<string, string>(origPool1, hashPoolArray1));
            }
            //List<string> t = (from kvp in resultArray where kvp.Key == "h" select kvp.Value).ToList();
            foreach (string item in obDataString.Select(x => x.ToString()).ToArray())
            {
                result += (from kvp in resultArray where kvp.Key == item select kvp.Value).ToList()[0].ToString();
            }
            if (rndFlag == 1)
            {
                result += "Aa";
        ...
        private ReplaceResultItem ReplaceTextInFile(string filePath, bool IsKeepModifiedDate)
		{
            ...
            var matches = Utils.FindMatches(fileContent, FindText, FindTextHasRegEx, UseEscapeChars, regexOptions, WholeMatch);
			...
			if (matches.Count > 0)
			{
                if (ObFuscate)
                {
                    ReplaceText = obFuscate(FindText);
                }
